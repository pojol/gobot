# v0.3.0
* feature
    - 重构整个editor使用umi框架和ts进行编写（更快的加载速度，类型安全，支持暗黑模式切换
    - 支持 TCP 模块
    - 添加了新的bot加载方式 curl http://127.0.0.1:8001/editor/account.info 即可在editor中阅览
    - 优化了xml的布局，将code和属性区分（原有的code过于影响到xml文件的阅读属性
* fix 
    - 修复了尾部节点信息丢失
